// Day 1 — net match play. Win = 2 pts, Tie = 1 pt, Loss = 0.
// Match is decided on total net strokes over 18 holes.
...
export type Day1MatchInput = {
  ...
  strokesGiven: number;
  strokeHoles: number[]; // hole numbers on which the giver receives a stroke (giver's net = gross - 1)
};
...
export type Day1MatchResult = {
  ...
  currentHolesUp: number; // + = p1 up, - = p2 up (net)
};
...
export function computeDay1MatchResult(
  ...
  const strokeSet = new Set(match.strokeHoles);
  ...
  for (let h = 1; h <= 18; h++) {
    ...
    if (s1 != null && s2 != null) {
      holesBothPlayed += 1;
      const hasStroke = strokeSet.has(h);
      const p1Net = s1 - (hasStroke && match.strokeGiverId === match.p1Id ? 1 : 0);
      const p2Net = s2 - (hasStroke && match.strokeGiverId === match.p2Id ? 1 : 0);
      if (p1Net < p2Net) currentHolesUp += 1;
      else if (p2Net < p1Net) currentHolesUp -= 1;
    }
  }
  ...
  return {
    ...
    p1TotalNet: allPlayed ? p1TotalNet : p1Holes > 0 ? p1TotalNet : null,
    p2TotalNet: allPlayed ? p2TotalNet : p2Holes > 0 ? p2TotalNet : null,
    winnerId,
    p1TeamPoints,
    p2TeamPoints,
    status,
